...
- Changing the type of a field (e.g., `int` → `string`)
- Adding a new required field to a request body
- Changing a response status code (e.g., 200 → 201)
...
## 6. Skipping Versions
Clients may skip intermediate versions. V1 → V3 is allowed. Clients are not forced to migrate through every intermediate version. However, each version has its own sunset window, so skipping versions may result in a shorter effective migration period.
